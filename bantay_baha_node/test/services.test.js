...
import { collectSource } from "../src/services/collector.js";
...
test("live collection skips an overlapping run without fetching",async()=>{let released=false;const connection={execute:async(sql)=>{assert.match(sql,/GET_LOCK/);return [[{acquired:0}]];},release:()=>{released=true;}};const db={getConnection:async()=>connection};const result=await collectSource("pdrrmo",{},db);assert.deepEqual(result,{status:"skipped",reason:"overlap",source:"pdrrmo"});assert.equal(released,true);});
